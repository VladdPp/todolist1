...
func SetupPostgres() {
	...
	db, err = sql.Open("postgres", "postgres://vlad:J9B4bDHl2d3TZ5yEWzg10vYU78xAk6tG@185.226.42.66/todolist?sslmode=disable")
...
func CreateTodoItem(c *gin.Context) {
	...
	if len(item) == 0 {
		...
		// Insert item to DB
		_, err := db.Query("INSERT INTO list(item) VALUES($1);", TodoItem.Item)
		if err != nil {
			fmt.Println(err.Error())
			c.JSON(http.StatusInternalServerError, gin.H{"message": "error with DB"})
